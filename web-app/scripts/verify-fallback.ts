...
async function main() {
  const url = "https://www.nasa.gov/missions/webb/nasa-documentary-cosmic-dawn-reveals-untold-story-of-james-webb-space-telescope/";
  ...
  dispatcher.get(target.origin).intercept({ path: target.pathname }).reply(200,
  ...
  try {
    ...
    const report = {
      testedAt: new Date().toISOString(), url,
      ...
      pipeline: { method: article.extractionMethod, fallback: article.usedFallback, contentLength: article.content.length, title: article.title, authors: article.authors },
    };
...
main().catch((error: unknown) => { console.error(error instanceof Error ? error.message : "Fallback verification failed"); process.exitCode = 1; });
