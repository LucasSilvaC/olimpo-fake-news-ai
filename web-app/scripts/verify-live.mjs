...
const cases = [
  ...
  [
    "JPL SSR / JSON-LD",
    "https://www.jpl.nasa.gov/news/nasa-webbs-autopsy-of-planet-swallowed-by-star-yields-surprise/",
  ],
...
const ui = {
  status: page.status,
  title: html.includes("News Parser"),
  input: html.includes('id="news-url"'),
  button: html.includes("Extract"),
  script: html.includes("/_next/static/"),
};
const results = [];
...
for (let offset = 0; offset < cases.length; offset += 2) {
  results.push(
    ...(await Promise.all(
      cases.slice(offset, offset + 2).map(async ([name, url]) => {
        const start = Date.now();
        try {
          const response = await fetch(`${base}/api/news/extract`, {
            method: "POST",
...
await writeFile(
  ...
  JSON.stringify({ testedAt: new Date().toISOString(), base, ui, results }, null, 2),
);
