...
export async function POST(request: Request): Promise<Response> {
  try {
    ...
    } catch { throw new ExtractionError("INVALID_URL"); }
    const parsed = extractNewsSchema.safeParse(body);
    if (!parsed.success) throw new ExtractionError("INVALID_URL");
    return Response.json(await extractNews(parsed.data.url), { headers: { "Cache-Control": "no-store" } });
  } catch (error) {
    const failure = error instanceof ExtractionError ? error : new ExtractionError("INTERNAL_ERROR");
