...
export function parseJsonLd(document: Document, base: string): ArticleFields {
  ...
  const rank = (node: Record<string, unknown>) =>
    Math.min(
      ...types(node)
        .map((type) => supported.indexOf(type))
        .filter((index) => index >= 0),
    );
  const score = (node: Record<string, unknown>) =>
    useful.filter((key) => Boolean(node[key])).length;
