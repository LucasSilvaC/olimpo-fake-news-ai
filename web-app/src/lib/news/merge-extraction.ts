...
export function mergeExtraction(url: string, json: ArticleFields = {}, meta?: MetadataFields, reader: ArticleFields = {}): INewsArticle {
  const jsonContent = json.content?.trim() ?? "";
  const readerContent = reader.content?.trim() ?? "";
  const content = readerContent.length >= 300 && (jsonContent.length < 300 || readerContent.length > jsonContent.length * 1.2)
    ? readerContent : jsonContent || readerContent;
  return {
    url,
    canonicalUrl: json.canonicalUrl ?? meta?.canonicalUrl ?? null,
    title: json.title ?? meta?.title ?? reader.title ?? meta?.documentTitle ?? meta?.twitterTitle ?? null,
    description: json.description ?? meta?.description ?? reader.description ?? meta?.metaDescription ?? meta?.twitterDescription ?? null,
    authors: json.authors?.length ? json.authors : meta?.authors?.length ? meta.authors : reader.authors ?? [],
    ...
    extractionMethod: "local", usedFallback: false,
  };
}

export function isExtractionUsable(article: INewsArticle): boolean {
  return article.content.trim().length >= 300 && Boolean(article.title?.trim() || article.publisher?.trim() || article.authors.some((author) => author.trim()));
}
...
export function mergeFallback(local: INewsArticle, fallback: ArticleFields): INewsArticle {
  return {
    ...
    extractionMethod: "jina", usedFallback: true,
  };
}
