...
export const resolveAddresses: DnsLookup = (hostname) => lookup(hostname, { all: true, verbatim: true });
...
export function isPublicAddress(address: string): boolean {
  try {
    ...
    if (ip.kind() === "ipv6" && !(ip as ipaddr.IPv6).match(ipaddr.IPv6.parse("2000::"), 3)) return false;
...
export function parsePublicUrl(input: string): URL {
  let url: URL;
  try { url = new URL(input); } catch { throw new ExtractionError("INVALID_URL"); }
  if (!["http:", "https:"].includes(url.protocol)) throw new ExtractionError("INVALID_URL");
  const host = url.hostname.replace(/^\[|\]$/g, "").replace(/\.$/, "").toLowerCase();
  if (url.username || url.password || url.port || !host.includes(".") && !isIP(host)
...
export async function validateUrl(input: string, signal: AbortSignal, resolver: DnsLookup = resolveAddresses) {
