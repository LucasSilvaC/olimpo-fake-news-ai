...
- Implementar uma arquitetura modular Feature-First em `src/app/api/{feature}` encapsulando `entities`, `usecase`, `repositories`, `actions` e `tests`.
- Manter o Repository Pattern com Injeção de Dependência (DI) em cada feature.
- Centralizar conexões compartilhadas (Drizzle DB client e Redis singleton) em `src/server/shared`.
...
- **Decisão:** Implementar a feature `auth` em `src/app/api/auth/`:
...
### 2. Organização em Camadas Feature-First (`src/app/api/{feature}`)
- **Decisão:** Estruturar o backend modularmente por feature dentro de `src/app/api/{feature}/`. Cada feature é autocontida contendo suas próprias camadas e testes:
  ```text
  src/app/api/{feature}/
...
- **Decisão:** Implementar `MockAIAnalysisService` dentro de `src/app/api/ai-feedback`. O mock avalia a notícia, retorna a classificação alvo e gera 2 a 3 argumentos analíticos simulados, permitindo testes end-to-end do fluxo antes de plugar um LLM real.
