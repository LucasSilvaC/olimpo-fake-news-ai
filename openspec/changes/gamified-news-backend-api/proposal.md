...
- **Arquitetura Modular Feature-First**: Organização de cada fatia em `src/app/api/{feature}` contendo `entities`, `usecase`, `repositories`, `actions` e `tests`.
...
- **Código & Camadas**:
  - `src/app/api/{feature}`: Fatias modulares para cada funcionalidade (`auth`, `rooms`, `news-voting`, `ai-feedback`, `global-challenges`), cada uma contendo `entities/`, `usecase/`, `repositories/`, `actions/` e `tests/`.
  - `src/server/shared`: Conexões e clients compartilhados (Drizzle ORM client, schemas e Redis singleton).
